  ...
  <div id="rateLimitDiv">
    <table class="table table-bordered">
      ...
      <tr v-for="rateLimit in rateLimitList" v-bind:key="rateLimit">
        ...
        <td>{{rateLimit.qps}}</td>
        <td><button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modifyRateLimit">修改</button><button type="button" v-on:click="deleteRateLimitConfig(rateLimit.id, rateLimit.api)" class="btn btn-danger">删除</button></td>
      </tr>
      </tbody>
    </table>
    <div><button type="button" style="float:right" class="btn btn-success" data-toggle="modal" data-target="#addRateLimit">新增</button></div>
      ...
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn-success" data-dismiss="modal" v-on:click="updateRateLimitConfig">
              确认
            </button>
          </div>
        </div>
    ...
    <div class="modal fade" id="addRateLimit" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-hidden="true">
          ...
          <div class="modal-body">
            <form class="bs-example bs-example-form" role="form">
              ...
              <div class="input-group">
                <span class="input-group-addon">QPS</span>
                <input id="addQps" type="text" class="form-control" placeholder="">
              </div>
            </form>
          </div>
          <div class="modal-footer">
            ...
            <button type="button" class="btn btn-success" data-dismiss="modal" v-on:click="addRateLimitConfig">
              确认
            </button>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    addRateLimitConfig: function () {
      var id = $("#addId").val()
      var api = $("#addApi").val()
      var rateKey = $("#addKey").val()
      var qps = $("#addQps").val()
      if (typeof rateKey == "undefined" || rateKey == null || rateKey == "") {
        alert("限流key不能为空")
        return
    ...
    deleteRateLimitConfig: function (id, api) {
      const axios = require('axios').default
      axios
        .get('http://localhost:8100/gateway/delete-rate-limit-config', {
          params: {
            id: id,
            api: api
          }
...
$(function () {
  ...
  $('#addRateLimit').on('show.bs.modal', function (event) {
    $("#addId").val("")
    $("#addApi").val("")
    $("#addKey").val("")
    $("#addQps").val("")
